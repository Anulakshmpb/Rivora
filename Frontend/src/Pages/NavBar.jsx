...
export default function NavBar() {
    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 font-inter py-3 px-4 ${
            isScrolled ? 'bg-white/70 backdrop-blur-lg border-b border-gray-100 shadow-sm' : 'bg-transparent'
        }`}>
            <div className={`max-w-[1440px] mx-auto px-6 py-4 flex justify-between items-center transition-all rounded-[2rem] ${
                !isScrolled ? 'bg-white shadow-2xl m-2' : ''
            }`}>
                {/* Mobile Menu Toggle */}
                <button 
                ...
                <div className="flex items-center gap-4 lg:gap-7">
                    ...
                    <button className="hidden lg:block ml-2 text-xs font-black uppercase tracking-widest text-gray-400 hover:text-red-900 transition-colors">
                        Logout
                    </button>
                </div>
            </div>
            ...
            {/* Mobile Menu Sidebar */}
            <div className={`fixed top-0 left-0 h-screen w-[280px] bg-white z-[52] shadow-2xl transition-transform duration-300 lg:hidden ${
