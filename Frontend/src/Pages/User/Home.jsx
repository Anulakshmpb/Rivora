...
function Home() {
	...
	const fetchSiteReviews = async () => {
		try {
			const res = await axiosInstance.get('/api/reviews?type=site');
			if (res.success) {
				setSiteReviews(res.reviews);
			}
		} catch (error) {
				...
				<div className="max-w-7xl mx-auto">
					<div className="text-center mb-12 animate-fade-in">
						...
						<button
							...
							className="bg-black text-white px-8 py-3 rounded-full font-black text-xs uppercase tracking-widest hover:bg-gray-800 transition-all duration-300 shadow-xl shadow-gray-200"
						>
							Share Your Story
						</button>
					</div>
					...
					<div className="relative group/slider">
						...
						>
							{siteReviews.map((item, index) => (
								<div
									key={index}
									className="flex-shrink-0 w-full md:w-[45%] lg:w-[31%] snap-center"
								>
									<div className="group relative flex flex-col h-full p-8 rounded-[2rem] bg-white border border-gray-100 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.1)] hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] transition-all duration-500 hover:-translate-y-2">
