...
export default function Checkout() {
    const { cartItems, cartTotalPrice, cartTotalItems, clearCart } = useCart();
    const { user } = useAuth();
    ...
    const handleApplyCoupon = async () => {
        ...
        try {
            ...
            showToast('Success', `Coupon applied: ${res.data.data.discount}% OFF`, 'success');
        } catch (err) {
            showToast('Error', err.response?.data?.message || 'Invalid coupon', 'error');
                    ...
                    <div className="flex-1 space-y-12">
                        ...
                        <section>
                            ...
                            <div className="space-y-4">
                                ...
                                >
                                    <div className="flex items-center gap-6">
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${paymentMethod === 'razorpay' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600'}`}>
                                ...
                                >
                                    <div className="flex items-center gap-6">
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${paymentMethod === 'cod' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600'}`}>
                                ...
                                <button 
                                    ...
                                    className={`w-full p-6 rounded-[2rem] border-2 transition-all flex items-center justify-between group ${paymentMethod === 'others' ? 'border-blue-600 bg-white shadow-xl shadow-blue-900/5' : 'border-slate-100 bg-slate-50/50 hover:bg-white'}`}
                                >
                                    <div className="flex items-center gap-6">
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${paymentMethod === 'others' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600'}`}>
                                        ...
                                        <div className="text-left">
                                            <h3 className="font-black text-slate-900 text-base">Other Options</h3>
                                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">Wallet, EMI, etc.</p>
                                        </div>
                                    </div>
                                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${paymentMethod === 'others' ? 'border-blue-600' : 'border-slate-200'}`}>
                                        {paymentMethod === 'others' && <div className="w-3 h-3 rounded-full bg-blue-600" />}
                                    </div>
                                </button>
                            </div>
                        </section>
                            ...
                            {cartItems.map((item) => (
                                <div key={item.id} className="flex gap-4">
                                    ...
                                    <div className="flex-1 flex flex-col justify-center">
                                        ...
                                        <p className="text-sm font-black text-slate-900 mt-2">${item.product.price.toFixed(2)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
